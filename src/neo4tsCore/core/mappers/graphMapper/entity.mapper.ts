...
export class EntityMapper {
    ...
    getEntitiesFromDtoArray(inputDtos:GraphAbstraction):IGraphEntity[]{
        ...
        const [nodesGraphs, relationshipGraphs] = this.getArrayOfGraphAbstraction(inputDtos);
    ...
    private getArrayOfGraphAbstraction(inputDtos: GraphAbstraction): [AliasedNode[], AliasedRelationship[]] {
        ...
        const nodesGraphs: AliasedNode[] = aliasedGraph.filter(e => 
            !((e[1] as GraphRelationship).source || (e[1] as GraphRelationship).from) &&
            !((e[1] as GraphRelationship).target || (e[1] as GraphRelationship).to)
        ) as AliasedNode[];

        const relationshipGraphs: AliasedRelationship[] = aliasedGraph.filter(e => 
            ((e[1] as GraphRelationship).source || (e[1] as GraphRelationship).from) ||
            ((e[1] as GraphRelationship).target || (e[1] as GraphRelationship).to)
    ...
    private getInputNode([alias, graphNode]: AliasedNode): Node {
        ...
        const found:IGraphEntity|undefined = this.wasEntityCreated(alias, id);

        if ( found ) return found as Node;
        else return this.nodeMapper.getInputNode([alias, graphNode]);
    }

    private getInputRelationship([alias, graphRelationship]: AliasedRelationship): Relationship {
        const id:string|number|undefined = EntityUtils.getId(graphRelationship);
        const found:IGraphEntity|undefined = this.wasEntityCreated(alias, id);

        if ( found ) return found as Relationship;
        else return this.relationshipMapper.getInputRelationShipWithEntities([alias, graphRelationship], this.entitiescreated)
    }

    private wasEntityCreated(alias: string, id: string|number|undefined): IGraphEntity|undefined {
        let found;
