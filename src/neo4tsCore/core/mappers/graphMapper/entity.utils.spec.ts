...
describe('testing entity utils', () => {
    ...
    describe('testing getId from graph entity', () => {
        ...
        test('test when sending entity with properties ptSystemRelationshipId', () => {
            const graph: GraphEntity = {properties: {ptSystemRelationshipId: 1}};
            const result: string|number|undefined = getId(graph);
        ...
        test('test when sending entity with properties as object', () => {
            const graph: GraphEntity = {properties: {id: {type: 'number', value: 1}}};
            const result: string|number|undefined = getId(graph);
    ...
    describe('testing get labels', () => {
        ...
        test('when passing both label and labels should throw error', () => {
            ...
            try{
                ...
                expect(e).toEqual(new Error('Two identifiers for labels were found, you should use one of them (label or labels) not both'))
            }
        });
    ...
    describe('Testing pushGroupToAllProperties', () => {
        ...
        test('when passing a property with a group, the group gets concatenated', () => {
            const properties: Record<string, any> = {prop: {group: 'testg'}};
