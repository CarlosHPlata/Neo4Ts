...
export const getId = (graphEntity: GraphEntity):string|number|undefined => {
    ...
    if (graphEntity.id) {
        ...
        idValue = graphEntity.properties.id || graphEntity.properties.ptSystemNodeId || graphEntity.properties.ptSystemRelationshipId;
    }
...
export const getLabels = (graphEntity: GraphEntity):string[] => {
    if (graphEntity.labels && graphEntity.label){
        throw new Error('Two identifiers for labels were found, you should use one of them (label or labels) not both');
    }
...
export const pushGroupToAllProperties = (properties:Record<string, any>, newGroupName: string) => {
