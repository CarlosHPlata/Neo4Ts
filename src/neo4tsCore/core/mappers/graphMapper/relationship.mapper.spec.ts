...
describe('testing relationship mapper', () => {
    ...
    test('when sending an empty relationship it returns a valid response', () => {
        const result: Relationship = relationshipMapper.getInputRelationShipWithEntities([alias, relationship], entitiesCreated);
    ...
    test('when sending target unknow it creates an empty Node', () => {
        ...
        relationshipMapper.getInputRelationShipWithEntities([alias, relationship], entitiesCreated);

        expect(entitiesCreated.length).toBe(3);
        expect(entitiesCreated.some(e => e instanceof Node && e.alias === 'unknow')).toBeTruthy();
    });
    ...
    test('when passing unknown source and target it throws an error', () => {
        ...
        try {
            ...
            expect(e).toEqual(new Error('at least one of the relationship nodes (source or target) must exists in dtos list'))
        }
    });
