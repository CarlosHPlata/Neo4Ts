...
describe('testing property mapper', () => {
    ...
    test('passing a property with primitive value is converted to property', () => {
        const result: Property[] = propertyMapper.getPropertiesFromDto(properties);
    ...
    test('passing an array property returns a array type property', () => {
        ...
        const array1Prop: Property|undefined = result.find(p => p.alias === 'array1');
        const array2Prop: Property|undefined = result.find(p => p.alias === 'array2');
        const array3Prop: Property|undefined = result.find(p => p.alias === 'array3');
    ...
    test('passing an array with differente values will throws an error', () => {
        ...
        try {
            ...
            expect(e).toEqual(new Error('Array property not valid, arrays can contains only ONE primitive type'));
        }
    });

    test('passing an object as a property will return a valid property', () => {
        properties = {prop: {foo: 'bar'}};
        jest.spyOn(propertyObjectMapper, 'getPropertyIfObject').mockImplementation(() => 
