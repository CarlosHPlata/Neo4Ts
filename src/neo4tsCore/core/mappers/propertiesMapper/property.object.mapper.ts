...
export class PropertyObjectMapper {
    ...
    protected propertyValueTypeFactory: (alias: string, dbprop: GraphPropertyDefinition) => Property;
    ...
    getPropertyIfObject(alias: string, dbprop: GraphPropertyDefinition|any): Property {
        if ( dbprop.type != null && dbprop.value != null ) {
            return this.buildObjectPropertyWithTypeAndValue(alias, dbprop);

        } else if ((dbprop.type && !dbprop.value) || (!dbprop.type && dbprop.value)){
            throw new Error('Can\'t parse a property causevalue or type is null or undefined');
    ...
    private buildObjectPropertyWithTypeAndValue(alias: string, dbprop: GraphPropertyDefinition):Property {
        try{
            return this.buildObjectProperty(alias, dbprop);
        } catch (e){
            throw new Error('error parsing object in PropertyProvider for key: '+alias);
        }
    }

    private buildObjectProperty(alias: string, dbprop: GraphPropertyDefinition):Property{
    ...
    private addPropertyDecoratorsFromDto(prop: Property, dto: GraphPropertyDefinition):Property {
        if (dto.condition){
    ...
    private buildObjectPropertyAsJson(alias: string, dbprop: any):Property {
        ...
        return new Property(alias, PropertyTypes.STRING, JSON.stringify(dbprop));
    }


    private generateNewFilterGroups(property:Property, groupsString:string):string[]{
        ...
        for (let newGroup of newGroups) {
            ...
            else throw new Error(`Error generating filter groups, an invalid group was found`);
        }
